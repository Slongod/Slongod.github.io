"use strict";
function render(content, display, displaylines) {
    const sign = display ? '$$' : '$';
    content = content.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
    if (displaylines)
        return `${sign}\\displaylines{${content}}${sign}`;
    else
...
function math_plugin(md, options) {
    ...
    md.renderer.rules.math_inline = (tokens, idx) => { return render(tokens[idx].content, false, inline); };
    md.renderer.rules.math_block = (tokens, idx) => { return render(tokens[idx].content, true, block); };
}
;
